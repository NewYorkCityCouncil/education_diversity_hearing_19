...
```{r}
school_dems %>% 
  filter(dbn %in% c("15K130", "17K249"), year == "2017-18") %>% 
  select(school_name,
  ...
  mutate_if(is.numeric, percent) %>% 
  gather(var, val, - school_name) %>% 
  spread(school_name, val)

diversity_report_k8 %>% 
  filter(dbn %in% c("15K130", "17K249"), category == "All Students") %>% 
  select(school_name, ela_percent_l3_l4, math_percent_l3_l4) %>% 
